...
import { Async_Core_Stream_Uint8_Read_Lines } from './lib/ericchase/Core_Stream_Uint8_Read_Lines.js';
...
if (targetset.size > 0) {
  ...
  await Async_Core_Stream_Uint8_Read_Lines(Bun.spawn(['lstree', dirpath], { stdout: 'pipe' }).stdout, (line) => {
    dirlist.push(line.slice(2));
  });
  clearInterval(tid);
  console.write('.');
  console.log();
